...
{
    public partial class DeletePotentialInfluenceForm : Form
    {
        MainForm mainform;
        ChangeDB_Lib.Change_Interaction change_obj;

        public DeletePotentialInfluenceForm()
        {
            InitializeComponent();
        }

        private void DeletePotentialInfluenceForm_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void but_ok_Click(object sender, EventArgs e)
        {
            try
            {
                if (tb_ch_param.Text == "" || tb_name_action.Text == "" || tb_var_name.Text == "") throw new Exception("Все поля должны быть заполнены!");
                else
                {
                    change_obj.DeletePotInf(tb_ch_param.Text, tb_name_action.Text, tb_var_name.Text);
