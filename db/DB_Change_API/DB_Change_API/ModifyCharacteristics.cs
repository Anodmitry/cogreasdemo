...
{
    public partial class ModifyCharacteristics : Form
    {
        MainForm mainform;
        ChangeDB_Lib.Change_ActObj change_obj;

        public ModifyCharacteristics()
        {
            InitializeComponent();
        }

        private void ModifyActObjCharacteristics_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void but_ok_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (rb_add.Checked) change_obj.AddCharacteristic(tb_name.Text);
                if (rb_delete.Checked) change_obj.DeleteCharacteristic(tb_name.Text);
                if (rb_change.Checked)
                {
                    if (tb_new_name.Text == "") throw new Exception("Все поля должны быть заполнены!");
                    change_obj.ChangeCharacteristic(tb_name.Text, tb_new_name.Text);
                }
                this.Close();
            }
            catch (Exception ex)
